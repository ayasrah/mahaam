...
func TrafficMiddleware(trafficRepo repo.TrafficRepo, cfg *conf.Conf, logger logs.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if c.Request.Body != nil && cfg.LogReqEnabled {
		...
		defer func() {
			...
			if !strings.HasPrefix(path, "/swagger") && !strings.EqualFold(path, "/health") && !strings.HasPrefix(path, "/audit") {
				...
				go func() {
					...
					defer func() {
						if r := recover(); r != nil {
							logger.Error(trafficId, "Traffic creation failed: %v", r)
						}
					}()
