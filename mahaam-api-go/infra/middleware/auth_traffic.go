...
func AuthMiddleware(authService *security.AuthService, logRepo repo.LogRepo) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if appStore == "" || appVersion == "" {
			...
			logRepo.Create(trafficId, "Error", "Required headers not exists")
			return
		}
		...
		if requiresAuth {
			var err error
			userId, deviceId, err = authService.ValidateAndExtractJwt(c)
			if err != nil {
				c.AbortWithStatusJSON(http.StatusUnauthorized, "invalid jwt")
				logRepo.Create(trafficId, "Error", err.Error())
				return
			}
			c.Set("userId", userId)
