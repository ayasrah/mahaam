...
export class DefaultHealthRepo implements HealthRepo {
  async create(): Promise<string> {
    const id = randomUUID();
    await DB.sql`INSERT INTO x_health (id, api_name, api_version, env_name, node_ip, node_name, started_at) VALUES (${id}, ${config.apiName}, ${config.apiVersion}, ${config.envName}, ${Cache.getNodeIP()}, ${Cache.getNodeName()}, current_timestamp)`;
    return id;
  }
  ...
  async updatePulse(id: string): Promise<void> {
    await DB.sql`UPDATE x_health SET pulsed_at = current_timestamp WHERE id = ${id}`;
  }

  async updateStopped(id: string): Promise<void> {
    await DB.sql`UPDATE x_health SET stopped_at = current_timestamp WHERE id = ${id}`;
  }
}
